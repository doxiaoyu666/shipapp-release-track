...
export function CrashDetail({ appId, appName }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/apps/${appId}/crashes/trend?days=${days}`)
      .then((r) => r.json())
    ...
    <div>
      ...
      {buildSummaries.length > 0 && (
        <div className="charts-grid" style={{ marginBottom: 24 }}>
          {buildSummaries.slice(0, 4).map((b, i) => {
